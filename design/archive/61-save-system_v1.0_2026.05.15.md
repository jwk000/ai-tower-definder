...
archived-reason: v3.4 形态级重构第 2 轮 —— 存档契约 v2.0.0 → v3.0.0（删除 shardBalance/cardCollection/permanentUpgrades/OngoingRun/CardEntry.techTree 等 meta 字段）
supersededBy: ../60-tech/61-save-system.md  # v3.0.0 单 Run 闭环存档契约
authority-for:
  - save-format-v1  # 仅供历史回溯，不再作为需求来源
  - persistence-spec-v1
cross-refs:
...
# 存档系统（v1.0 历史快照，已被 v3.0.0 完全重写）

> 🛑 **本文档已归档，不再作为需求来源**（2026-05-15）：
>
> - v3.4 形态级重构第 2 轮已将 61-save-system 从 v1.0 完全重写为 v3.0.0，存档契约从 v2.0.0 升至 v3.0.0
> - 本快照保留 v1.0 完整内容（455 行 / 11 节），仅供回溯 v3.0/v3.1 时期的存档结构（含 SparkShards / CardCollection / PermanentUpgrades / OngoingRun / CardEntry.techTree 等 meta 字段）
> - **当前权威**：[../60-tech/61-save-system.md](../60-tech/61-save-system.md) v3.0.0
> - **变更主声明**：[../v3.4-MAJOR-MIGRATION.md](../v3.4-MAJOR-MIGRATION.md) §3.2 第 5 项

---

## 以下为 v1.0 原始内容（无修改）

> 🛑 **v3.4 形态级重构中（2026-05-15）**：本文档**核心存档结构已被部分废弃**：
>
> - **`SparkShards`**（火花碎片余额）：v3.4 **彻底废弃**字段
> - **`CardCollection`**（永久卡池解锁状态）：v3.4 **彻底废弃**字段（所有卡开局即解锁，无需持久化）
> - **`PermanentUpgrades`**（关外永久升级）：v3.4 **彻底废弃**字段
> - **`CardEntry.techTree`** / 塔科技树持久化：v3.4 **彻底废弃**（技能树本 Run 临时状态，不持久化）
> - **v3.4 新增**：存档版本升至 **v3.0.0**（游戏未发布，无迁移代码），存档仅持久化"主菜单设置项 + 战绩统计"等无 meta 资源数据
> - **新开发请优先参考**：[v3.4-MAJOR-MIGRATION §4.2](../v3.4-MAJOR-MIGRATION.md#42-代码层不变式第-4-轮生效)
> - 本文档将于 v3.4 第 2 轮全面改写。
