...
| S20 | ~~`main.ts` Wave 6 wire 不接 `HandSystem` / `DeckSystem` / `EnergySystem` / `CardSpawnSystem` / `CardRegistry`、不连任何 UI 输入回调~~ | 10-roguelike-loop §2.3-§2.7 / 40-ui-ux §3 / 48-shop §3 | `src/main.ts` | ✅ **已替换（W7.4e）**：Wave 7 全栈装配接入所有 unit-system + setHandler 回调 |
| S21 | ~~`projectUIFrame()` 把 `HUD.RunState.phase` 字段写死 `'battle'`~~ | 40-ui-ux §2 HUD / 10-roguelike-loop §4 | `src/main.ts` projectUIFrame / `src/ui/HUD.ts` | ✅ **已替换（W7.3+W7.5）**：接 LevelState.phase，W7.5 集成测试覆盖相位序列 |
| S22 | ~~`projectUIFrame()` 把 `HandState` 投空集合 `{ cards: [], energy: 0 }`~~ | 10-roguelike-loop §2.3 / 40-ui-ux §3 | `src/main.ts` projectUIFrame | ✅ **已替换（W7.4e）**：接 HandSystem.cards + CardRegistry 查 cost/playable + EnergySystem.current |
| S23 | ~~`main.ts` 不注册 `drop_gold` RuleHandler、不实例化 `EconomySystem`~~ | 10-roguelike-loop §2.5 / 11-economy §2 | `src/main.ts` / `src/systems/EconomySystem.ts` | ✅ **已替换（W7.4e+W7.6）**：drop_gold handler 注册 + economy 装配；W7.6 集成测试端到端覆盖 |
| S24 | ShopPanel.onBuy / MysticPanel.onMysticChoice / SkillTreePanel.onSpendSP 3 个 UI 回调仅 wire 空函数 stub，不接真实逻辑 | 48-shop §3 / 27-traps-spells §5 / 22-skill-tree §7 | `src/main.ts`（无 wire，dev-hook 暴露） | 🟡 **Wave 8 跟进**：MVP 演示路径仅需 start-run + play-card + return-to-menu 3 条主路径 |
| S25 | MovementSystem path 仍使用 level.path（loader 投影直线，非 pathGraph 动态路由）；pathGraph 多段路径支持推迟 | 13-map-level §2 pathGraph / 16-level-blueprints §1 | `src/main.ts` / `src/systems/MovementSystem.ts` | 🟡 **Wave 8 跟进**：level-01 单一直线路径，loader 投影结果与 DEFAULT_PATH 等价，不影响 MVP 演示 |
...
| **2.7** | **2026-05-16** | **Wave 7 完成**：8 atomic commits（W7.1→W7.8）全部落地。WaveSystem（spawn cadence + 相位状态机）+ ProjectileSystem（飞行弹道 + TTL + 命中扣血）+ LevelState（HUD 相位桥接）+ W7.4 全栈装配（main.ts 129→274 行：YAML 静态导入、5 unit-system、7 system pipeline、drop_gold handler、4 panel class setHandler、projectUIFrame 取 levelState.phase）+ W7.5 HUD.phase 集成测试（phase 序列 deployment→battle→wave-break×2→victory）+ W7.6 drop_gold 端到端测试 + W7.7 性能基线（avg 0.010ms ≪ 16.67ms 目标）。287/287 测试全绿 / typecheck / build 三命令门通过。S20-S23 全部替换；新增 S24（3 UI 回调 stub）/ S25（MovementSystem 用 level.path 直线，pathGraph 动态路由 Wave 8 跟进）。下一步 Wave 8：Pixi 事件链真实绑定 + pathGraph 接入 + 内容扩展 |
