...
| `Trap` | 陷 | 地面机关（玩家可部署的卡牌产物） | 尖刺陷阱、地雷、火墙、引力场 |
| `Scene` | 景 | **关卡预置的互动机关 / 环境威胁**（不可由卡牌部署） | 火药桶、墓碑、藤蔓、诅咒神龛 |
| `Neutral` | 中 | 中立资源点（可被双方争夺） | 泉水、宝箱、法力水晶、远古祭坛 |
| `Objective` | 标 | 目标点 | 基地、出生点 |

> `Scene` 与 `Neutral` 的区别：`Scene` 是**关卡设计师在 levelEditor 中预置**的、与关卡主题强绑定的固定机关或环境威胁；`Neutral` 是关卡随机生成的可争夺资源点。两者均不可由玩家从手牌出卡。详见 [27-traps-spells-scene §4](./27-traps-spells-scene.md#4-场景中立单位scene-neutral)。
